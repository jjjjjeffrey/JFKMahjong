...
enum MahjongTile: CustomStringConvertible {
    ...
    var discardedLeftImageName: String {
        get {
            switch self {
            case let .rank(number, rank):
                return "\(rank.rawValue)-left-\(number)"
            case let .wind(wind):
                return "\(wind.rawValue)-left"
            case let .dragon(dragon):
                return "\(dragon.rawValue)-left"
            case let .flower(flower):
                return "\(flower.rawValue)-left"
            }
        }
    }
    
    var discardedTopImageName: String {
        get {
            switch self {
            case let .rank(number, rank):
                return "\(rank.rawValue)-top-\(number)"
            case let .wind(wind):
                return "\(wind.rawValue)-top"
            case let .dragon(dragon):
                return "\(dragon.rawValue)-top"
            case let .flower(flower):
                return "\(flower.rawValue)-top"
            }
        }
    }
    
    var discardedRightImageName: String {
        get {
            switch self {
            case let .rank(number, rank):
                return "\(rank.rawValue)-right-\(number)"
            case let .wind(wind):
                return "\(wind.rawValue)-right"
            case let .dragon(dragon):
                return "\(dragon.rawValue)-right"
            case let .flower(flower):
                return "\(flower.rawValue)-right"
            }
        }
    }
